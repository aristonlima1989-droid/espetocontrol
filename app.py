...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS produtos (
        ...
        tipo_produto TEXT DEFAULT 'AVULSO',
        ativo INTEGER DEFAULT 1
    )""")
    ...
    if "tipo_produto" not in colunas_produtos:
        cur.execute("ALTER TABLE produtos ADD COLUMN tipo_produto TEXT DEFAULT 'AVULSO'")
    ...
    # MIGRACAO_V21 - normalização dos tipos comerciais
    cur.execute("UPDATE produtos SET tipo_produto='AVULSO' WHERE tipo_produto IS NULL OR tipo_produto=''")
    cur.execute("UPDATE produtos SET tipo_produto='COMPLETO' WHERE tipo_produto='COMBO'")
    conn.commit()
    conn.close()
...
def lancar_produto_ou_combo(conn, comanda_id, produto_id, quantidade, ponto_carne, observacao):
    ...
    if produto["tipo_produto"] in ("SIMPLES", "COMPLETO"):
...
def dashboard():
    ...
    pedidos_pendentes = conn.execute("""
        ...
        FROM (
            ...
            WHERE p.status IN ('PENDENTE','EM PREPARO')
              AND c.status = 'ABERTA'
              AND COALESCE(pr.tipo_produto, 'AVULSO') NOT IN ('SIMPLES','COMPLETO')
        ) x
    """).fetchone()["total"]

    pedidos_prontos = conn.execute("""
        SELECT COUNT(*) AS total
        FROM (
            SELECT DISTINCT p.comanda_id, pr.setor_preparo
            FROM pedidos p
    ...
    vendas_dia = conn.execute("""
        ...
        FROM pagamentos
        WHERE substr(data_pagamento,1,10)=?
    """, (datetime.now().strftime("%Y-%m-%d"),)).fetchone()["total"]

    comandas_fechadas_dia = conn.execute("""
        SELECT COUNT(*) total
        FROM pagamentos
        WHERE substr(data_pagamento,1,10)=?
    """, (datetime.now().strftime("%Y-%m-%d"),)).fetchone()["total"]

    ticket_medio = (vendas_dia / comandas_fechadas_dia) if comandas_fechadas_dia else 0
    ...
    if request.method == "POST":
        ...
        try:
            conn.execute("""
                ...
                WHERE id=?
            """, (nome, login_novo, perfil, ativo, usuario_id))
            conn.commit()
...
@app.route("/usuario/inativar/<int:usuario_id>")
@login_required
@perfil_required("ADMIN")
def inativar_usuario(usuario_id):
    ...
    if usuario["login"] == "admin":
        conn.close()
        flash("O usuário admin principal não pode ser inativado.", "danger")
        return redirect(url_for("usuarios"))
...
@app.route("/usuario/reativar/<int:usuario_id>")
@login_required
@perfil_required("ADMIN")
...
@app.route("/usuario/resetar_senha/<int:usuario_id>", methods=["GET","POST"])
@login_required
@perfil_required("ADMIN")
def resetar_senha_usuario(usuario_id):
    ...
    if request.method == "POST":
        nova_senha = request.form.get("senha") or "123456"
        conn.execute("UPDATE usuarios SET senha=? WHERE id=?", (generate_password_hash(nova_senha), usuario_id))
        conn.commit()
        conn.close()
    ...
    conn.close()
    return render_template("resetar_senha_usuario.html", usuario=usuario)
...
def combo_config(combo_id):
    conn = conectar()
    combo = conn.execute("SELECT * FROM produtos WHERE id=? AND tipo_produto IN ('SIMPLES','COMPLETO')", (combo_id,)).fetchone()
    if not combo:
    ...
    itens = conn.execute("""
        ...
        WHERE ci.combo_id=? ORDER BY pr.setor_preparo, pr.descricao
    """, (combo_id,)).fetchall()
    produtos = conn.execute("SELECT * FROM produtos WHERE ativo=1 AND COALESCE(tipo_produto, 'AVULSO') IN ('AVULSO','BEBIDA') AND id<>? ORDER BY setor_preparo,descricao", (combo_id,)).fetchall()
    conn.close()
    ...
    conn.execute("""
        ...
        WHERE comanda_id = ?
          ...
          AND produto_id IN (
              ...
              WHERE setor_preparo = ?
                AND COALESCE(tipo_produto, 'AVULSO') NOT IN ('SIMPLES','COMPLETO')
          )
...
def api_app_health():
    return jsonify({"status":"ok","sistema":"EspetoControl","versao":"2.1","mensagem":"API disponível"})
